...
import { POLYGON_BLOCK_TIME, DEFAULT_TOKEN_DECIMAL } from 'config'
...
const getDateForBlock = async (currentBlock: number, block: number) => {
  ...
  const secondsUntilStart = blocksUntilBlock * POLYGON_BLOCK_TIME
...
export const processAuctionData = async (auctionId: number, auctionResponse: AuctionsResponse): Promise<Auction> => {
  ...
  const blocksToFarmStartDate = hoursToSeconds(12) / POLYGON_BLOCK_TIME
  const farmStartBlock = processedAuctionData.endBlock + blocksToFarmStartDate
  const farmDurationInBlocks = hoursToSeconds(7 * 24) / POLYGON_BLOCK_TIME
